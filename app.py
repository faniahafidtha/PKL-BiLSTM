        ...
        if st.sidebar.button("🚀 Buat Prediksi", use_container_width=True):
            ...
            if not sektor_terpilih:
                ...
                for i, sektor in enumerate(sektor_terpilih):
                    ax = axes[i]
                    display_name = SEKTOR_MAPPING.get(sektor, sektor)
                    ...
                    ax.set_title(f"Tren dan Prediksi Sektor {display_name}")
                    ax.legend()
                ...
                with st.expander("Lihat Prediksi Detail per Provinsi dan Unduh Hasil"):
                    ...
                    kolom_untuk_format = [SEKTOR_MAPPING.get(s) for s in sektor_terpilih if s in df_summary_provinsi.columns]
                    st.dataframe(df_summary_provinsi_display.style.format(formatter="{:,.2f}", subset=kolom_untuk_format), use_container_width=True)
                    # ==================================
